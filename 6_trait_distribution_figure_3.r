...
facet_grid(cols=vars(Country),rows=vars(type))+scale_color_viridis(option="mako")+scale_fill_viridis(option="mako")+coord_cartesian(expand=F)

####Panel b
forb=as.data.frame(ini_net %>% group_by(site,min_transect_elev,Country) %>% summarise(compl=mean(-1*abs(trait_plant-match_infer)),prop_forbidden=mean(1-average_proba),
