...
elev_img <- get_elev_raster(ex.df, prj = CRS("EPSG:4326"), z = 8, clip = "bbox")
bloc=st_as_sf(as.data.frame(rasterToPoints(elev_img)),coords=c("x","y"),crs=CRS("EPSG:4326"))
names(bloc)[1]="elev"
...
bbox_centroid<- data.frame(x = c(mean(st_coordinates(pts1)[,1]),mean(st_coordinates(pts2)[,1])), y = c(mean(st_coordinates(pts1)[,2]),mean(st_coordinates(pts2)[,2]))) %>%
...
pl1=ggplot()+
geom_sf(data=b,aes(color=alwdgg),size=0.3)+
...
scale_color_distiller(palette = "Spectral",limits=c(-50,max(b$alwdgg)),n.breaks=4)+xlim(c(-1710854,6242145))+labs(col="Elev. (m)")+
